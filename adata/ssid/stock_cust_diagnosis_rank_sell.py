...
class StockCustDiagnosisRankSell(LeekSparkJob):
    ...
    def _cal_percent_rank(self, dfBase, col_name, order):
        ...
        df2 = dfBase.where("%s is null" % col_name) \
             .withColumn(col_name + "_rank", fn.lit(None)) \
            .persist(storageLevel=StorageLevel.DISK_ONLY)
        df.union(df2)
        return df

    def _cal(self, df, lo_col, hi_col, order1="asc", order2="desc", type="buy"):
        ...
        dfInd = df.select("trade_id", "compute_term", "busi_date", "avg_chg_industry_3d",
                          "avg_chg_industry_5d", "avg_chg_industry_10d", "top_ind_perc_3d",
                          "top_ind_perc_5d", "top_ind_perc_10d", "bottom_ind_perc_3d",
                          "bottom_ind_perc_5d", "bottom_ind_perc_10d",
                          )
        ...
        dfInd = self._cal_percent_rank(dfInd, "bottom_ind_perc_3d", order1)
        dfInd = self._cal_percent_rank(dfInd, "bottom_ind_perc_5d", order1)
        dfInd = self._cal_percent_rank(dfInd, "bottom_ind_perc_10d", order1)
        dfPrd = df.select("trade_id", "compute_term", "busi_date", "avg_chg_stock_3d",
                          "avg_chg_stock_5d", "avg_chg_stock_10d", "top_stock_perc_3d",
                          "top_stock_perc_5d", "top_stock_perc_10d", "bottom_stock_perc_3d",
                          "bottom_stock_perc_5d", "bottom_stock_perc_10d")
        ...
        dfPrd = self._cal_percent_rank(dfPrd, "bottom_stock_perc_3d", order1)
        dfPrd = self._cal_percent_rank(dfPrd, "bottom_stock_perc_5d", order1)
        dfPrd = self._cal_percent_rank(dfPrd, "bottom_stock_perc_10d", order1)
    ...
    def _get_base_data(self, startdate, enddate, compute=90):
        sql = """
            ...
            cast(sum(case when ind_rank_perc_3d>=0.9 then 1.0 else 0 end)/
                  count(ind_rank_perc_3d) as double) bottom_ind_perc_3d,
            cast(sum(case when ind_rank_perc_5d>=0.9 then 1.0 else 0 end)/
                  count(ind_rank_perc_5d) as double) bottom_ind_perc_5d,
            cast(sum(case when ind_rank_perc_10d>=0.9 then 1.0 else 0 end)/
                  count(ind_rank_perc_10d) as double) bottom_ind_perc_10d,
            ...
            cast(sum(case when stock_rank_perc_3d>=0.9 then 1.0 else 0 end)/
                  count(stock_rank_perc_3d) as double) bottom_stock_perc_3d,
            cast(sum(case when stock_rank_perc_5d>=0.9 then 1.0 else 0 end)/
                  count(stock_rank_perc_5d) as double) bottom_stock_perc_5d,
            cast(sum(case when stock_rank_perc_10d>=0.9 then 1.0 else 0 end)/
                  count(stock_rank_perc_10d) as double) bottom_stock_perc_10d,
            '{1}' busi_date,
            {5} compute_term,
            trade_id
            from {6}.{7}
            ...
            GROUP BY trade_id
        """
        sellSql = sql.format(startdate, enddate, 2, "avg_sell2low", "avg_high2sell",
                             compute, self.adata, self.stock_cust_trd_quant)
        sellBaseDf = self.sparkSession.sql(sellSql)
        sellBaseDf = self._add_average(sellBaseDf, "avg_sell2low", "avg_high2sell",
                                       enddate, compute)
        sellBaseDf = self._cal_top(sellBaseDf)
        sellBaseDf.persist(StorageLevel.DISK_ONLY).count()
        return sellBaseDf
    ...
    def _add_average(self,df, low_col, high_col, enddate, compute_term):
        avgDf = df.agg(
            ...
            fn.avg("bottom_ind_perc_3d").alias("bottom_ind_perc_3d"),
            fn.avg("bottom_ind_perc_5d").alias("bottom_ind_perc_5d"),
            fn.avg("bottom_ind_perc_10d").alias("bottom_ind_perc_10d"),
            ...
            fn.avg("bottom_stock_perc_3d").alias("bottom_stock_perc_3d"),
            fn.avg("bottom_stock_perc_5d").alias("bottom_stock_perc_5d"),
            fn.avg("bottom_stock_perc_10d").alias("bottom_stock_perc_10d")
    ...
    def _cal_top(self, df, table_type='time'):
        ...
        df = df.withColumn("top_ind_perc_3d",
            ...
            .persist(storageLevel=StorageLevel.DISK_ONLY)
        df.count()
        df = df.withColumn("bottom_ind_perc_3d",
                        fn.when(df.avg_chg_industry_3d == None, fn.lit(None)).
                        otherwise(df.bottom_ind_perc_3d)) \
            .withColumn("bottom_ind_perc_5d",
                        fn.when(df.avg_chg_industry_5d == None, fn.lit(None)).
                        otherwise(df.bottom_ind_perc_5d)) \
            .withColumn("bottom_ind_perc_10d",
                        fn.when(df.avg_chg_industry_10d == None, fn.lit(None)).
                        otherwise(df.bottom_ind_perc_10d)) \
            .persist(storageLevel=StorageLevel.DISK_ONLY)
        df.count()
        df = df.withColumn("top_stock_perc_3d",
            ...
            .persist(storageLevel=StorageLevel.DISK_ONLY)
        df.count()
        df = df.withColumn("bottom_stock_perc_3d",
                        fn.when(df.avg_chg_stock_3d == None, fn.lit(None)).
                        otherwise(df.bottom_stock_perc_3d)) \
            .withColumn("bottom_stock_perc_5d",
                        fn.when(df.avg_chg_stock_5d == None, fn.lit(None)).
                        otherwise(df.bottom_stock_perc_5d)) \
            .withColumn("bottom_stock_perc_10d",
                        fn.when(df.avg_chg_stock_10d == None, fn.lit(None)).
                        otherwise(df.bottom_stock_perc_10d))\
            .persist(storageLevel=StorageLevel.DISK_ONLY)
        df.count()
        return df
    ...
    def daily_compute(self, startdate=None, enddate=None, compute=90):
        ...
        daily_compute
        """
        sellBaseDf = self._get_base_data(startdate, enddate, compute)
        dfSellTime, dfSellInd, dfSellPrd =\
            self._cal(sellBaseDf, "avg_sell2low", "avg_high2sell", "desc", "asc")
        self.save_rank_table(compute, enddate,
                             dfSellTime.repartition(20), self.stock_cust_diagnosis_sell_time)
        self.save_rank_table(compute, enddate,
                             dfSellInd.repartition(20), self.stock_cust_diagnosis_sell_ind)
        self.save_rank_table(compute, enddate,
                             dfSellPrd.repartition(20), self.stock_cust_diagnosis_sell_prd)
