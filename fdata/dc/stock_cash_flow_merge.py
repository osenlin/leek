...
class StockCashFlowMerge(LeekSparkJob):
    ...
    def init_data(self):
        sql = """
            create table if not exists {0}.{1} (
                ...
                STORED AS ORC
        """.format(self.fdata, self.conf['stock_cash_flow_merge_table'])
        print sql
    ...
    def daily_compute(self, startdate=None, enddate=None):
        df = self.sparkSession.sql("""
            ...
            """.format(enddate, self.odata, self.conf['cash_flow_table']))
        save_data(self.sparkSession, self.fdata, self.conf['stock_cash_flow_merge_table'],
                  enddate, df)
