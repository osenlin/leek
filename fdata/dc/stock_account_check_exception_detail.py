...
class StockAccountCheckExceptionDetail(LeekSparkJob):
    ...
    def init_data(self):
        sql = """
            create table if not exists {0}.{1} (
                ...
                STORED AS ORC
        """.format(self.fdata, self.conf['check_data_exception_table'])
    ...
    def daily_compute(self, startdate=None, enddate=None):
        self.sparkSession.sql(
            ...
            .format(enddate, self.fdata, self.conf['check_data_exception_table']))
        ...
        self.sparkSession.sql("""
            ...
            select * from {3}
        """.format(enddate, self.fdata, self.conf['check_data_exception_table'], temptable))
        self.sparkSession.sql(
            "refresh table %s.%s" % (self.fdata, self.conf['check_data_exception_table']))
