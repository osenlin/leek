...
class StockAccountCheckExceptionStat(LeekSparkJob):
    ...
    def __init__(self, spark):
        ...
        self.stock_daily_check_report = self.conf['hk_stock_daily_check_report']
        self.hk_check_data_table = self.conf['hk_check_data_table']
    ...
    def init_data(self):
        sql = """
          create table if not exists {0}.{1} (
                ...
                STORED AS ORC
        """.format(self.fdata, self.stock_daily_check_report)
    ...
    def daily_compute(self, startdate=None, enddate=None):
        sqlPrePartition = " alter table {1}.{2} add if not exists  partition(busi_date= '{0}') "\
            .format(enddate, self.fdata, self.stock_daily_check_report)
        ...
        """.format(enddate, self.fdata, self.hk_check_data_table)
        ...
        self.sparkSession.sql("""
            ...
            select * from {3}
        """.format(enddate, self.fdata, self.stock_daily_check_report, temptable))
        self.sparkSession.sql(
            "refresh table %s.%s" % (self.fdata, self.stock_daily_check_report))
