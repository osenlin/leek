...
class StockAccountCheck(LeekSparkJob):
    ...
    def __init__(self, spark):
        ...
        self.stock_cash_flow = self.conf['hk_stock_cash_flow_merge_table']
        self.check_data_table = self.conf['hk_check_data_table']
        self.asset_table = self.conf['hk_asset_table']
        self.debt_table = self.conf['hk_debt_table']
    ...
    def init_data(self):
        sql = """
            create table if not exists {0}.{1} (
                    ...
                    STORED AS ORC
        """.format(self.fdata, self.check_data_table)
    ...
    def daily_compute(self, startdate=None, enddate=None):
        ...
        self.sparkSession.sql(
            ...
            .format(endDate, self.fdata, self.check_data_table))
        ...
        checkSqlTmp = """
            select
                ...
                ft.busi_flag_code,
                ft.return  return,
                case when ft.prd_no='hk.fee' or (ft.ignore_rr_check=1 and ft.return_rate is NULL)
                  then 0 else ft.return_rate end return_rate,
                case when ft.prd_no='hk.fee' then 0 else ft.qty_exception end qty_exception,
                case when ft.prd_no='hk.fee'  or ft.return_rate is not null or 
                    ft.ignore_rr_check=1
                    then 0 else 1 end return_rate_exception,
                trd_type
            from (
                ...
                greatest(nvl(fb.ignore_rr_check,0),nvl(oa.ignore_rr_check,0)) ignore_rr_check,
                '{5}' trd_type
                from (
        ...
        longCheckSql = checkSqlTmp.format(
            ...
            "qty", beginDate, endDate, "{0}.{1}".format(self.odata, self.asset_table),
            "long_related",
            "{0}.{1}".format(self.fdata, self.stock_cash_flow))
        shortCheckSql = checkSqlTmp.format(
            """(nvl(now_mkt_val,0)-nvl(pre_mkt_val,0)-nvl(trd_cash_flow,0))/
                (nvl(pos_cash_flow,0)-nvl(now_mkt_val,0))""",
            "liab_qty", beginDate, endDate, "{0}.{1}".format(self.odata, self.debt_table),
            "short_related",
            "{0}.{1}".format(self.fdata, self.stock_cash_flow))
        ...
        insertSql = """
            ...
            select * from {3}
        """
        self.sparkSession.sql(insertSql.format("overwrite", "{0}.{1}".format(
            self.fdata, self.check_data_table), endDate, longTempTable))
        ...
        self.sparkSession.sql(
            insertSql.format("into", "{0}.{1}".format(self.fdata, self.check_data_table),
                             endDate, shortTempTable))
        self.sparkSession.catalog.dropTempView(shortTempTable)
        self.sparkSession.sql("refresh table %s.%s" % (self.fdata, self.check_data_table))
