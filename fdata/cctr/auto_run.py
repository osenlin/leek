...
def run_local(mode, load_date):
    ...
    """
    sparkHome = [
        "/Users/lzh/spark/spark-2.2.0-bin-hadoop2.7/bin/spark-submit"]
    master = ["--master", "local[*]"]
    scriptPath = ["%s/bin/daily_update.py" % homePath]
    pyPackage = ["--py-files", "%s/lib/leek.zip" % homePath]
    command = sparkHome + master + pyPackage + scriptPath + [mode, load_date]
    runCommand(command)
...
def run_cluster(mode, load_date):
    ...
    master = ["--master", "yarn"]
    resource = ["--num-executors", "8", "--executor-cores", "2",
                "--executor-memory", "4g"]
    scriptPath = ["%s/bin/daily_update.py" % homePath]
    pyPackage = ["--py-files", "%s/lib/leek.zip" % homePath]
    command = sparkHome + master + resource\
        + pyPackage + scriptPath + [mode, load_date]
    runCommand(command)
